...
- 点击小区后，以交通红高亮到北沙滩站的高德实际步行路线，显示米数、预计分钟、路线起点类型和当前库存。
...
- 房源卡片：4:3 主图 + 卡内左右切图（箭头/滑动）与“第 n/N 张”，懒加载，失败占位；卡片本体不误触跳走，只有“查看贝壳原房源”按钮（新标签，`noopener noreferrer`）跳转。
...
- 步行距离和时间来自高德 Web 服务实际路径规划，不使用直线距离冒充步行距离。
